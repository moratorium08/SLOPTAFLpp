...
The results of the FuzzBench benchmark in our paper are left in `./benchs` of the `benchmark` branch, and those of the Magma benchmark can be downloaded from [here](https://drive.google.com/file/d/1ejOgMKBtbB_ynwAWbkpZZ-DMiyltwKqg/view?usp=sharing).
Note that the size of the latter one is big (about 65GB), so we prepared [here](https://drive.google.com/file/d/1fC72oJmFF51MLR5LryYr5i3oPRe81S66/view?usp=sharing) lists of bugs that were found during our experiments so that you can check our results instantly. 
This list was generated by using [exp2json.py](https://hexhive.epfl.ch/magma/docs/technical.html#exp2jsonpy).
